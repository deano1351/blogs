...
### The DATE Function
The `DATE` function gives the sequential serial number that represents a particular date. We use the `DATE` function when we take three different values and merge them to form a date.
...
- DAY
The `DAY` argument represents day of the month from 1 - 31. If you use a value more than 31 for this argument, it will be added to the first day of the month given as the second argument. For instance, if you use 32 in the third argument **`=DATE(2005, 11, 32)`** in excel, it will give output as **`02-12-2005`**.

- MONTH
The `MONTH` argument represents the month of the year from 1 - 12. If you use a value more than 12 for this argument, it will be added to the first month of the year given as the first argument. For instance, if you use 16 in second argument **`=DATE(2005, 16, 2)`** in excel, it will give output as **`02-04-2006`**.

- YEAR
The `YEAR` argument can contain one to four digits. It is always better to use four digits as the argument in the function to avoid any kind of confusion, as Microsoft Excel for Windows uses the 1900 date system by default. So, if you use one digit in first argument **`=DATE(1, 11, 2)`** in Excel, it will give output as **`02-11-1901`**. So, it is always a good practice to use four digits in the first argument. 
...
The `DATEVALUE` function is used to convert date that is available as text to a serial number that will be recognized as a date by Excel. 
...
### The DAY Function
The `DAY` function returns the day of the given date or serial number and it ranges from 1-31.
...
### The EDATE Function
The `EDATE` function gives the serial number/ date of the date before or after a specified date (the `start_date`). 
...
Now, let us put the formula **`=EDATE(B1,C1)`** in cell `D1` and then apply it in subsequent rows. This function will return the date after adding the **MONTH** value to the date mentioned in the **DATE** and the resulting value will be updated as below:
...
The `EOMONTH` function gives the last day of the month of the indicated number of months before or after the `start_date` as the serial number.
...
Therefore, let us put the formula **`=EOMONTH(B1,C1)`** in cell `D1` and then apply it in subsequent rows. The resulting value will be updated as below:
...
Therefore, let us put the formula **`=HOUR(B1)`** in cell `C1` and then apply it in subsequent rows. The resulting value will be updated as below:
...
The `ISOWEEKNUM` function gives the ISO (International Organization for Standards) week number of the year for a given date.
...
Next, let us put the formula **`=ISOWEEKNUM(B1)`** in cell `C1` and then apply it in subsequent rows. The resulting value will be updated as below:
...
The `MINUTE` and `MONTH` function converts the serial number into a minute and month respectively.
...
Next, let us put the formula **`=MINUTE(C1)`** in cell `D1` and  **`=MONTH(B1)`** in cell `E1` and then apply it in subsequent rows. The resulting value will be updated as below:
...
## Conclusion
In this guide, you've learned the first set of date and time operations in Excel like DATE, DATEVALUE, DAY, DAYS, etc. In the [second](/guides/working-with-time-date-functions-excel-part-2) part of this guide series, you'll learn about the next set of date and time operations.
